#!/usr/bin/env perl
################################################################################
# Log-Viewer REST-API v1.3.0 (asynchron)
#
# Beschreibung:
#   REST-API für hec zum sicheren Anzeigen und Durchsuchen von Log-Dateien.
#   Unterstützt ACL, API-Token, asynchrone Dateiausgabe und detailliertes Logging.
#
# Features:
#   - Asynchroner tail-Aufruf (Mojo::IOLoop->subprocess)
#   - IP/CIDR- und Token-basierte Authentifizierung
#   - Schutz vor Directory-Traversal und Symlinks
#   - Dateityp-Whitelisting
#   - CORS-Unterstützung
#   - HTTPS-Unterstützung
...
# ------------------ Defaults & Hardening ------------------
umask 0007;  # Dateien 0660 / Verzeichnisse 0770 als Standard
...
die "Config JSON ungueltig: $@" if $@ || ref($Config) ne 'HASH';
...
# ------------------ Helpers: IP ACL (CIDR) ------------------
# IPv4 CIDR (a.b.c.d/nn) + einzelne IPv4
# IPv6: nur exact match (weil ohne Zusatzmodule)
sub _ipv4_to_int {
  my ($ip) = @_;
...
# Async Variante: liefert Promise, Route bleibt gleich (Output identisch)
sub slurp_tail_utf8 {
  my ($lines, $file) = @_;
  $lines //= 2000;
...
get '/log/:name' => sub {
  ...
  my $entry = $logdirs->{$name} or return fail_json($c, "Unbekanntes Logverzeichnis: $name", 404);
  my $base_abs = $LOGBASE{$name} // return fail_json($c, "Pfad nicht zugänglich: $name", 500);

  my $dir = path($base_abs);
  return fail_json($c, "Kann Verzeichnis nicht oeffnen: $base_abs", 500) unless -d $dir->to_string;
...
get '/log/*name/*file' => sub {
  ...
  # Async lesen, Output bleibt 1:1 gleich
  $c->render_later;
